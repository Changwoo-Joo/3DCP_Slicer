...
# =========================
# G-code generator (수정: 단일선 모드시 ref→선 시작 / 선 끝→ref travel 추가)
# =========================
def generate_gcode(mesh, z_int=30.0, feed=2000, ref_pt_user=(0.0, 0.0),
                   ...
                   box_right_edge_only: bool = False):
    ...
    for z in z_values:
        ...
        if box_right_edge_only:
            ...
            if is_box and line_poly is not None:
                ...
                box_detected = True
        if not layer_polys:  # 일반 모드 or 박스 판정 실패
        ...
        # === 수정 포인트 ===
        # 박스 단일선이면: (A) ref→선시작 travel → (B) 선 가공 → (C) 선끝→ref travel → (D) 다음 레이어 ref에서 시작
        if box_detected:
            line = layer_polys[0]
            if line.shape[0] >= 2:
                sx, sy = float(line[0,0]), float(line[0,1])
                ex, ey = float(line[-1,0]), float(line[-1,1])
                rx, ry = float(ref_pt_layer[0]), float(ref_pt_layer[1])

                # (A) ref → 선 시작
                g.append(f"G01 F{feed}")
                g.append(f"G01 X{rx:.3f} Y{ry:.3f} Z{z:.3f}")
                g.append(f"G01 X{sx:.3f} Y{sy:.3f} Z{z:.3f}")

                # (B) 선 가공
                g.append(f"G01 F{feed}")
                if start_e_on and e_on:
                    g.append(f"G01 X{sx:.3f} Y{sy:.3f} Z{z:.3f} E{start_e_val:.5f}")
                for p2 in line[1:]:
                    if e_on:
                        # E 증분은 중심거리*계수. 단일선이라 간단히 처리
                        # (정확 누적 대신 구간별 증분만)
                        g.append(f"G01 X{p2[0]:.3f} Y{p2[1]:.3f} E{EXTRUSION_K * np.linalg.norm((p2[:2]-line[0,:2])):.5f}")
                    else:
                ...
                # (D) 다음 레이어 시작점을 ref로 고정
                prev_start_xy = np.array([rx, ry], dtype=float)
                continue  # 이 레이어 처리 끝. 다음 z로
        # === /수정 포인트 ===
        ...
        # 레이어 시작점 auto_start 기준 갱신
        prev_start_xy = layer_polys[0][0][:2]

        for i_seg, poly in enumerate(layer_polys):
            if poly.shape[0] < 2:
                continue
            if i_seg > 0:
                s = poly[0]
...
# =========================
# Slice path computation (수정: 단일선 모드시 ref 왕복 travel을 items에 반영)
# =========================
def compute_slice_paths_with_travel(
    ...
    for z in z_values:
        ...
        if auto_start and prev_start_xy is not None:
            ...
            ref_pt_layer = np.array(ref_pt_user, dtype=float)
        ref3 = np.array([ref_pt_layer[0], ref_pt_layer[1], z], dtype=float)
        ...
        if not layer_polys:  # 일반 모드 or 박스 감지 실패
            for seg3d in segments:
                seg3d_no_dup = ensure_open_ring(seg3d)
                shifted, _ = shift_to_nearest_start(seg3d_no_dup, ref_point=ref_pt_layer)
                trimmed = trim_closed_ring_tail(shifted, trim_dist)
        ...
        # 레이어 시작 travel (이전 레이어 끝→이번 레이어 ref)
        if prev_layer_last_end is not None:
            travel = np.vstack([prev_layer_last_end, ref3])
            all_items.append((travel, np.array([0.0, 0.0]) if e_on else None, True))

        if box_detected:
            # === 수정 포인트: ref↔line 왕복 travel 삽입 ===
            line = layer_polys[0]
            if line.shape[0] >= 2:
                start3 = line[0].copy()
                end3   = line[-1].copy()
                ...
                # (A) ref → 선 시작 travel
                travel1 = np.vstack([ref3, start3])
                all_items.append((travel1, np.array([0.0, 0.0]) if e_on else None, True))
                ...
                if e_on:
                    ...
                    for p1, p2 in zip(line[:-1], line[1:]):
                        ...
                        e_vals.append(total)
                    all_items.append((line, np.array(e_vals), False))
                else:
                    all_items.append((line, None, False))

                # (C) 선 끝 → ref travel
                travel2 = np.vstack([end3, ref3])
                all_items.append((travel2, np.array([0.0, 0.0]) if e_on else None, True))

                # (D) 레이어 종료점을 ref로 확정 (다음 레이어는 ref에서 시작)
                prev_layer_last_end = ref3.copy()
                prev_start_xy = ref3[:2].copy()
                continue
        # === /수정 포인트 ===

        # 일반 처리: 같은 레이어 내 경로 + intra-travel
        prev_start_xy = layer_polys[0][0][:2]
        # ref → 첫 경로 시작 travel
        first_poly_start = layer_polys[0][0]
        travel_in = np.vstack([ref3, first_poly_start])
        all_items.append((travel_in, np.array([0.0, 0.0]) if e_on else None, True))
        ...
        # 레이어 종료: 마지막 끝점 → ref 복귀 travel (다음 레이어는 ref에서 시작)
        last_end = layer_polys[-1][-1]
        travel_back = np.vstack([last_end, ref3])
        all_items.append((travel_back, np.array([0.0, 0.0]) if e_on else None, True))
        prev_layer_last_end = ref3.copy()
...
def compute_offsets_into_buffers(
    ...
    for i in range(N):
        ...
        r2 = (float(p2[0] - p1[0] - nx*half_width + p1[0]), float(p2[1] - p1[1] - ny*half_width + p1[1]), float(p2[2]))  # keep structure
