...
st.markdown(
    ...
    /* 탭과 뷰가 너무 위로 붙지 않도록 약간 내림 */
    .block-container { padding-top: 0.9rem; }

    /* 탭 상단에 여백 살짝 */
    .stTabs { margin-top: 0.25rem !important; padding-top: 0.1rem !important; }
    .stTabs [data-baseweb="tab-list"] { margin-top: 0.25rem !important; }

    /* 우측 컨트롤 패널: 독립 스크롤 + 살짝 내린 sticky 기준점 */
    .right-panel {
      position: sticky;
      top: 0.9rem;
...
# 새: 블록 형태(줄바꿈) 치수 포맷
def _fmt_dims_block(title, bbox, z_single: Optional[float]):
    if bbox is None:
        return f"**{title}**\nX=(-)\nY=(-)\nZ=(-)"
...
if segments is not None and total_segments > 0:
    ...
    if apply_offsets:
        ...
        dims_placeholder.markdown(_fmt_dims_block("외부치수", bbox_r, z_r))
    else:
