...
def trim_closed_ring_tail(segment: np.ndarray, trim_distance: float) -> np.ndarray:
    ...
    acc = 0.0; out = [ring[0].copy()]; i = 0
    while i < n and acc + lens[i] < target:
        acc += lens[i]; out.append(ring[(i + 1) % n].copy()); i += 1
...
def generate_gcode(mesh, z_int=30.0, feed=2000, ref_pt_user=(0.0, 0.0),
    ...
    for z in z_values:
        ...
        for i_seg, seg3d in enumerate(segments):
            ...
            if i_seg > 0:
                s = simplified[0]; g.append(f"G01 X{s[0]:.3f} Y{s[1]:.3f} Z{z:.3f}")
            start = simplified[0]; g.append(f"G01 F{feed}")
            if start_e_on:
            ...
            for p1, p2 in zip(simplified[:-1], simplified[1:]):
                ...
                if e_on: g.append(f"G01 X{p2[0]:.3f} Y{p2[1]:.3f} E{dist * EXTRUSION_K:.5f}")
                else:   g.append(f"G01 X{p2[0]:.3f} Y{p2[1]:.3f}")
            if e0_on: g.append("G01 E0")
            if i_seg == 0: prev_start_xy = start[:2]
...
def compute_slice_paths_with_travel(
    ...
    trim_dist=30.0, min_spacing=3.0, auto_start=False, e_on=False
    ...
    for z in z_values:
        ...
        for i_seg, seg3d in enumerate(segments):
            ...
            if i_seg == 0: prev_start_xy = simplified[0][:2]
        ...
        for i_seg in range(len(layer_polys)):
            ...
            if e_on:
                e_vals = [0.0]; total = 0.0
                for p1, p2 in zip(poly[:-1], poly[1:]):
                    dist = np.linalg.norm(p2[:2] - p1[:2]); total += dist * EXTRUSION_K
                    e_vals.append(total)
...
def append_segments_to_buffers(segments, start_idx, end_idx, stride=1):
    ...
    for i in range(start_idx, end_idx, max(1, int(stride))):
        ...
        if is_travel:
            if travel_mode == "hidden": continue
...
def compute_offsets_into_buffers(segments, upto, half_width, include_travel_climb=False, climb_z_thresh=1e-6):
    buf = st.session_state.paths_anim_buf
    buf["off_l"] = {"x": [], "y": [], "z": []}
    buf["off_r"] = {"x": [], "y": [], "z": []}
    ...
    for i in range(N):
        ...
        if (not is_travel) and is_extruding:
            ...
            dz = float(p2[2] - p1[2])
            if abs(dz) > climb_z_thresh: use_this = True
        if not use_this:
        ...
        dx = float(p2[0]-p1[0]); dy = float(p2[1]-p1[1]); nrm = (dx*dx+dy*dy)**0.5
        if nrm > 1e-12:
            tx, ty = dx/nrm, dy/nrm; prev_tan = (tx, ty)
        else:
...
def add_global_endcaps_into_buffers(segments, upto, half_width, samples=32, store_caps=False):
    ...
    first_idx = None; last_idx = None; N = min(upto, len(segments))
    for i in range(N):
        p1, p2, is_travel, is_extruding = segments[i]
        if (not is_travel) and is_extruding: first_idx = i; break
    for i in range(N-1, -1, -1):
        p1, p2, is_travel, is_extruding = segments[i]
        if (not is_travel) and is_extruding: last_idx = i; break
...
def ensure_paths_fig(height=820):
    fig = st.session_state.get("paths_base_fig"); ok = False
    try:
...
def _fmt_dims_inline(title, bbox, z_single: Optional[float]):
    if bbox is None:
        ...
        base = (f"**{title}**  X=({xm:.3f}→{xM:.3f}, Δ{xl:.3f}) · "
                f"Y=({ym:.3f}→{yM:.3f}, Δ{yl:.3f})")
    return base + (" · Z=(-)" if z_single is None else f" · Z=({z_single:.3f})")
...
def check_key_valid(k: str):
    if not k or k not in ALLOWED_WITH_EXPIRY: return False, None, None, "유효하지 않은 키입니다."
    exp = ALLOWED_WITH_EXPIRY[k]
    if exp is None: return True, None, None, "만료일 없음"
    try:
        exp_date = date.fromisoformat(exp)
    except Exception:
        return False, None, None, "키 만료일 형식 오류(YYYY-MM-DD)."
    today = date.today(); remaining = (exp_date - today).days
    if remaining < 0: return False, exp_date, remaining, f"만료일 경과: {exp_date.isoformat()}"
    elif remaining == 0: return True, exp_date, remaining, f"오늘 만료 ({exp_date.isoformat()})"
    else: return True, exp_date, remaining, f"만료일: {exp_date.isoformat()} · {remaining}일 남음"

KEY_OK, EXP_DATE, REMAINING, STATUS_TXT = check_key_valid(access_key)
if access_key:
    if KEY_OK:
        if EXP_DATE is None: st.sidebar.success(STATUS_TXT)
        else:
...
if uploaded is not None:
    ...
    if not isinstance(mesh, trimesh.Trimesh):
        st.error("STL must contain a single mesh"); st.stop()
    scale_matrix = np.eye(4); scale_matrix[2, 2] = 1.0000001  # Z 축 미세 확장
...
if KEY_OK and slice_clicked and st.session_state.mesh is not None:
    items = compute_slice_paths_with_travel(
        ...
        trim_dist=trim_dist, min_spacing=min_spacing, auto_start=auto_start, e_on=e_on
    )
...
def gcode_to_cone1500_module(gcode_text: str, rx: float, ry: float, rz: float) -> str:
    lines_out = []; cur_x = cur_y = cur_z = 0.0
...
if KEY_OK:
    ...
    if st.session_state.show_rapid_panel:
        with st.sidebar.expander("Rapid Settings", expanded=True):
            ...
            gtxt = st.session_state.get("gcode_text"); over = None
            if gtxt is not None:
                xyz_count = _extract_xyz_lines_count(gtxt); over = (xyz_count > MAX_LINES)
            ...
            if st.session_state.get("rapid_text"):
                base = st.session_state.get("base_name", "output")
                st.download_button(
                    "Rapid 저장 (.modx)", st.session_state.rapid_text,
    ...
    with tab_paths:
        if segments is not None and total_segments > 0:
            ...
            st.caption(
                ...
                + (f" | Caps 강조" if (st.session_state.get('apply_offsets_flag', False) and emphasize_caps) else "")
                + (f" | {travel_lbl}")
