...
st.markdown(
    ...
    /* 탭/컨텐츠가 화면 최상단에서 더 내려오도록 패딩 확대 */
    .block-container { padding-top: 1.6rem; }

    /* 탭 상단 여백 추가로 더 내려 보이게 */
    .stTabs { margin-top: 0.6rem !important; padding-top: 0.2rem !important; }
    .stTabs [data-baseweb="tab-list"] { margin-top: 0.4rem !important; }

    /* 우측 컨트롤 패널: 독립 스크롤 + sticky 기준점도 함께 내려줌 */
    .right-panel {
      position: sticky;
      top: 1.6rem;
...
def trim_closed_ring_tail(segment: np.ndarray, trim_distance: float) -> np.ndarray:
    ...
    while i < n and acc + lens[i] < target:
        acc += lens[i]
        out.append(ring[(i + 1) % n].copy()); i += 1
...
# Center + Right 레이아웃
# =========================
# ► 오른쪽 폭을 기존의 약 3/5로 축소 (기존 [5,2] ≈ 28.6% → [14,3] ≈ 17.6%)
...
if segments is not None and total_segments > 0:
    ...
    if apply_offsets:
        ...
        dims_placeholder.markdown(_fmt_dims_block_html("외부치수", bbox_r, z_r), unsafe_allow_html=True)
    else:
