...
def generate_gcode(mesh, z_int=30.0, feed=2000, ref_pt_user=(0.0, 0.0),
                   ...
                   skip_invalid_offset=True, use_centerline=False, wall_thickness=60.0):
...
def compute_slice_paths_with_travel(
    ...
    use_centerline=False, wall_thickness=60.0
...
def add_global_endcaps_into_buffers(segments, upto, half_width, samples=32, store_caps=False):
    ...
    def _append_arc(center, t_unit, n_unit, z, sign_t, steps):
        ...
        zs = np.full_like(xs, float(print_z))
...
# ── [벽체 중심선 생성 제어 UI 추가] ──
st.sidebar.subheader("벽체 중심선(Centerline) 제어")
use_centerline = st.sidebar.checkbox("벽체 중심 경로 생성 (Centerline)", value=False, help="벽체 외곽선 대신 중심 경로를 추출합니다.")
wall_thickness = st.sidebar.number_input("벽체 전체 두께 (mm)", min_value=1.0, max_value=2000.0, value=60.0, step=1.0, disabled=not use_centerline, help="벽체 총 두께의 절반만큼 안쪽으로 오프셋을 처리합니다.")
...
if st.session_state.get("access_key", ""):
    if KEY_OK:
        if EXP_DATE is None:
            ...
            data_mark = f"D-{REMAINING}" if REMAINING > 0 else "D-DAY"
            st.sidebar.info(f"{STATUS_TXT} ({data_mark})")
    else:
...
def _fmt_pos(v: float) -> str:
    ...
    s = f"{v:+.1f}"
    sign = s[0]
    data_intpart, dec = s[1:].split(".")
    data_intpart = data_intpart.zfill(4)
    return f"{sign}{data_intpart}.{dec}"
...
def _fmt_ang(v: float) -> str:
    ...
    sign = s[0]
    data_intpart, dec = s[1:].split(".")
    data_intpart = data_intpart.zfill(3)
    return f"{sign}{data_intpart}.{dec}" 

def _fmt_ext(v: float, z: int) -> str:
    if abs(v) < 5e-5: v = 0.0
    s = f"{abs(v):.1f}"
    data_intpart, dec = s.split(".")
    return f"{data_intpart.zfill(z)}.{dec}"
...
def _apply_const_speed_profile_on_nodes(
    ...
    data_use_step = (step_mm is not None) and (float(step_mm) > 0.0)
    snap_for_coord = _snap_step if data_use_step else _snap_linear
...
def convert_gcode_to_rapid(
    ...
    if bool(enable_a2_const):
        data_use_step = bool(st.session_state.get("extconsta2usestep", False))
        _apply_const_speed_profile_on_nodes(nodes, "a2", "raw_y", y_min, y_max, a2_at_ymin, a2_at_ymax, speed_mm_s, 11.0, boundary_eps_mm, apply_print_only, travel_interp,
                                            float(st.session_state.get("extconsta2stepmm", 0.0)) if data_use_step else 0.0)
    ...
    for nd in nodes:
        ...
        data_a1s, data_a2s, data_a3s, data_a4s = _fmt_ext(nd["a1"], 4), _fmt_ext(nd["a2"], 3), _fmt_ext(data_a3s_v, 3), _fmt_ext(data_a4s_v, 4)
        mat_val = "001" if nd["extr"] else "000"
        spd_val = f"{int(speed_mm_s):03d}"
        lines_out.append(f"{x},{y},{z},{frx},{fry},{frz},{data_a1s},{data_a2s},{data_a3s},{data_a4s},{mat_val},{spd_val}")
        ...
        with st.sidebar.expander("외부축 (A1/A2 등속 왕복 · 경계정지)", expanded=True):
            ...
            def edit_axis(title_key: str, axis_key: str):
                ...
                if axis_key in ("X", "Y"):
                    ...
                    a30 = cols_out[0].number_input("A3out0", value=float(PAX["A3_out"][0]), step=50.0, format="%.1f", key=f"{title_key}_{axis_key}_data_a30")
                    a31 = cols_out[1].number_input("A3out1", value=float(PAX["A3_out"][1]), step=50.0, format="%.1f", key=f"{title_key}_{axis_key}_data_a31")
                    PAX["A3_out"] = [float(a30), float(a31)]
                elif axis_key == "Z":
                    if "A4_out" not in PAX: PAX["A4_out"] = list(base.get("A4_out", [0.0, 0.0]))
                    a40 = cols_out[0].number_input("A4out0", value=float(PAX["A4_out"][0]), step=50.0, format="%.1f", key=f"{title_key}_{axis_key}_data_a40")
                    a41 = cols_out[1].number_input("A4out1", value=float(PAX["A4_out"][1]), step=50.0, format="%.1f", key=f"{title_key}_{axis_key}_data_a41")
...
with right_col:
    ...
    if segments is None or total_segments == 0:
        ...
        if max_layer_no > 0:
            ...
            data_c_layer1, data_c_layer2 = st.columns(2)
            with data_c_layer1:
