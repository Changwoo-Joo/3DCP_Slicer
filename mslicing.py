...
# --- NEW: 균등 아크길이 재샘플링 (형상 유지, XY 기준) ---
def resample_polyline_xy(points: np.ndarray, step: float) -> np.ndarray:
    """
    XY 평면에서 아크 길이 기준으로 'step' 간격으로 선형 보간해 재샘플.
    - 시작/끝점 보존
    - 중복/0길이 세그먼트 안전 처리
    - step <= 0이면 원본 반환
    """
    pts = np.asarray(points, dtype=float)
    if step <= 0.0 or len(pts) < 2:
        return pts

    seglens = [float(np.linalg.norm((q[:2] - p[:2]))) for p, q in zip(pts[:-1], pts[1:])]
    total = float(sum(seglens))
    if total <= 1e-12:
        return pts[[0, -1]] if len(pts) > 1 else pts

    num_steps = max(1, int(np.floor(total / step)))
    targets = [i * step for i in range(num_steps)]
    if targets[-1] < total:
    ...
    i = 0
    for t in targets[1:]:
        while i < len(seglens) and acc + seglens[i] < t - 1e-12:
            acc += seglens[i]
            i += 1
        if i >= len(seglens):
            break
        ...
        p1 = pts[i + 1]
        d = seglens[i]
        alpha = 0.0 if d <= 1e-12 else (t - acc) / d
        alpha = 0.0 if alpha < 0.0 else 1.0 if alpha > 1.0 else alpha
        interp = p0 + alpha * (p1 - p0)
        out.append(interp.copy())

    if np.linalg.norm(out[-1][:2] - pts[-1][:2]) > 1e-9:
        out.append(pts[-1].copy())

    if len(out) < 2:
        out = [pts[0].copy(), pts[-1].copy()]
...
def generate_gcode(mesh, z_int=30.0, feed=2000, ref_pt_user=(0.0, 0.0),
    ...
    for z in z_values:
        ...
        for i_seg, seg3d in enumerate(segments):
            ...
            # --- NEW: 강제 재샘플링 옵션 ---
            if force_resample:
                simplified = resample_polyline_xy(simplified, float(min_spacing))
...
force_resample = st.sidebar.checkbox(
    "Force points every 'Minimum point spacing' (mm)",
    value=False,
    help="체크 시, Minimum point spacing 값을 간격으로 XY 경로를 균등 아크길이 보간하여 형상을 유지하면서 포인트를 강제로 찍습니다."
)
...
if KEY_OK and slice_clicked and st.session_state.mesh is not None:
    items = compute_slice_paths_with_travel(
        ...
        e_on=e_on,
        force_resample=force_resample,  # NEW
    )
    ...
    with tab_paths:
        if segments is not None and total_segments > 0:
            ...
            st.caption(
                ...
                + (" | Resample: ON" if force_resample else " | Resample: OFF")
            )
        else:
            st.info("슬라이싱을 실행하세요.")
