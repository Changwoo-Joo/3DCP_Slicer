...
def _apply_const_speed_profile_on_nodes(
    ...
    # 1. 로컬 후진(역주행) 방지: 진행 방향과 반대로 값이 변하면 이전 값으로 고정
    mono_vals = [vals[0]]
    for v in vals[1:]:
        if is_increasing:
    ...
    # 2. 정지 구간(값이 멈춰있는 구간)을 선형 보간(Linear Interpolation) 처리
    #    -> 로봇이 멈추지 않고 부드럽게 이어가도록 대각선으로 펴줌
    i = 0
    while i < n_vals - 1:
    ...
    # 3. 보정된 값을 nodes에 다시 업데이트
    for idx in range(n_vals):
        nodes[idx][axis_key] = mono_vals[idx]


def gcode_to_cone_1500_module(
    gcode_text: str, rx: float, ry: float, rz: float, preset: Dict[str, Any],
    swap_a3_a4: bool = False,
    enable_a1_const: bool = True, enable_a2_const: bool = True,
    xmin: float = 0.0, xmax: float = 6000.0,
    a1_at_xmin: float = 4000.0, a1_at_xmax: float = 0.0,
    ymin: float = 0.0, ymax: float = 1000.0,
    a2_at_ymin: float = 0.0, a2_at_ymax: float = 4000.0,
    speed_mm_s: float = 200.0,
    ...
    def _g_id(d: Dict[str, Any], path: list, default: float) -> float:
        try:
            cur = d
    ...
    x0, x1 = _g_id(P, ["X","in",0], 0.0), _g_id(P, ["X","in",1], 1.0)
    y0, y1 = _g_id(P, ["Y","in",0], 0.0), _g_id(P, ["Y","in",1], 1.0)
    z0, z1 = _g_id(P, ["Z","in",0], 0.0), _g_id(P, ["Z","in",1], 1.0)
    a3_0, a3_1 = _g_id(P, ["Z","A3_out",0], 0.0), _g_id(P, ["Z","A3_out",1], 0.0)

    a4_on_x = "A4_out" in P.get("X", {})
    a4_on_y = "A4_out" in P.get("Y", {})
    a4x0, a4x1 = (_g_id(P, ["X","A4_out",0], 0.0), _g_id(P, ["X","A4_out",1], 0.0)) if a4_on_x else (0.0, 0.0)
    a4y0, a4y1 = (_g_id(P, ["Y","A4_out",0], 0.0), _g_id(P, ["Y","A4_out",1], 0.0)) if a4_on_y else (0.0, 0.0)
    ...
    for raw in gcode_text.splitlines():
        ...
        for p in t.split():
            if p.startswith("X"):
                try: cx = float(p[1:]); has_any = True; except: pass
            elif p.startswith("Y"):
                try: cy = float(p[1:]); has_any = True; except: pass
            elif p.startswith("Z"):
                try: cz = float(p[1:]); has_any = True; except: pass
            elif p.startswith("E"):
                try: ce = float(p[1:]); except: ce = None
        ...
        if not have_prev:
            if a4_on_x:
                cur_a4 = _linmap(cx, x0, x1, a4x0, a4x1)
            elif a4_on_y:
                cur_a4 = _linmap(cy, y0, y1, a4y0, a4y1)
            else:
                cur_a4 = 0.0
            have_prev = True
        else:
            ...
            if a4_on_x and abs(dx) > 0:
                cur_a4 += _prop_split_local(dx, x0, x1, a4x0, a4x1)
            elif a4_on_y and abs(dy) > 0:
                cur_a4 += _prop_split_local(dy, y0, y1, a4y0, a4y1)
    ...
    if len(xs_out) == 0:
        ...
        header = (
            ...
            "  !\n"
            "  ! data3dp : [Xmm, Ymm, Zmm, Rxdeg, Rydeg, Rzdeg, A1,A2,A3,A4]\n"
            "  !\n"
        )
        cnt_str = str(MAX_LINES)
        open_decl = (
            f"  VAR string sFileCount:=\"{cnt_str}\";\n"
    ...
    for i in range(len(xs_out)):
        nodes.append({
            "x": float(xs_out[i]), "y": float(ys_out[i]), "z": float(zs_out[i]),
            "raw_x": float(raw_xs[i]), "raw_y": float(raw_ys[i]),
            "a1": float(a1_list[i]), "a2": float(a2_list[i]), "a3": float(a3_list[i]), "a4": float(a4_list[i]),
            "extr": bool(is_extruding_list[i]),
        })

    if bool(enable_a1_const):
        _apply_const_speed_profile_on_nodes(
            nodes=nodes, axis_key='a1', coord_key='raw_x',
            coord_min=float(xmin), coord_max=float(xmax),
            axis_at_min=float(a1_at_xmin), axis_at_max=float(a1_at_xmax),
            speed_mm_s=float(speed_mm_s), eps_mm=float(boundary_eps_mm),
    ...
    if bool(enable_a2_const):
        ...
        if use_step:
            _apply_const_speed_profile_on_nodes(
                nodes=nodes, axis_key='a2', coord_key='coord_s',
            ...
            _apply_const_speed_profile_on_nodes(
                nodes=nodes, axis_key='a2', coord_key='raw_y',
                coord_min=float(ymin), coord_max=float(ymax),
                axis_at_min=float(a2_at_ymin), axis_at_max=float(a2_at_ymax),
                speed_mm_s=float(speed_mm_s), eps_mm=float(boundary_eps_mm),
                apply_print_only=bool(apply_print_only), travel_interp=bool(travel_interp),
    ...
    for nd in nodes:
        ...
        a3s = _fmt_pos(float(a3v))
        a4s = _fmt_pos(float(a4v))
    ...
    header = (
        ...
        "  !\n"
        "  ! data3dp : [Xmm, Ymm, Zmm, Rxdeg, Rydeg, Rzdeg, A1,A2,A3,A4]\n"
        "  !   A1 : raw_x (X based) boundary snap/hold, leave-boundary resumes\n"
        "  !   A2 : raw_y (Y based) boundary snap/hold, leave-boundary resumes\n"
        "  !\n"
    )
    cnt_str = str(MAX_LINES)
    open_decl = (
...
with right_col:
    ...
    apply_offsets = st.checkbox(
        ...
        help="Trim/Layer Width (mm) W 값 절반을 띄워서 그립니다.",
    ...
    include_z_climb = st.checkbox(
        ...
        value=True,
        help="Z로 올라가는 travel 구간도 오프셋 그리기 대상에 포함합니다.",
    ...
    emphasize_caps = st.checkbox(
        "Emphasize caps",
        value=False,
        help="양쪽 끝(캡) 부분을 빨간색 선으로 굵게 표시합니다.",
        disabled=(segments is None or not apply_offsets)
    )
    ...
    if e_on:
        ...
        st.checkbox("Show dotted travel lines", value=False, disabled=True, help="Insert E values가 OFF면 모든 travel 구간을 solid로 그립니다.")
        travel_mode = "solid"
    ...
    if segments is None or total_segments == 0:
        ...
        scrub = st.slider("스크러버 (segments)", 0, int(total_segments), int(default_val), 1, help="슬라이더")
        scrub_num = st.number_input("현재 표시 세그먼트 수", 0, int(total_segments), int(default_val), 1, help="숫자 입력")
        ...
        if layer_z is not None and layer_len is not None:
            st.caption(
                f"📌 현재 층: Z = {layer_z:.2f} mm\n\n"
                f"총 길이: {layer_len:.1f} mm ({layer_len/1000.0:.3f} m)"
            )
        else:
            st.caption("📌 현재 층: -")
...
if segments is not None and total_segments > 0:
    ...
    if bool(st.session_state.get("apply_offsets_flag", False)):
        ...
        zr = _last_z_from_buffer(st.session_state.paths_anim_buf["off_r"])
        dims_html = _fmt_dims_block_html("실제 치수 (우측 오프셋 기준)", bbox_r, zr)
        ...
        bbox_c = _bbox_from_buffer(st.session_state.paths_anim_buf["solid"])
        zc = _last_z_from_buffer(st.session_state.paths_anim_buf["solid"])
        dims_html = _fmt_dims_block_html("중심선 치수 (Solid)", bbox_c, zc)
        dims_placeholder.markdown(dims_html, unsafe_allow_html=True)
...
with center_col:
    ...
    with tab_paths:
        if segments is not None and total_segments > 0:
            ...
            if not e_on:
                travel_lbl = "Travel: solid (Insert E values OFF)"
            else:
                travel_lbl = "Travel: dotted" if tm == "dotted" else "Travel: hidden" if tm == "hidden" else "Travel: solid"

            st.caption(
                f"표시됨: {total_segments} 중 {st.session_state.paths_scrub} 세그먼트 | "
                ...
                f" | {travel_lbl}"
                f" | Viz stride={st.session_state.paths_anim_buf.get('stride',1)} (간소화 됨)" if st.session_state.paths_anim_buf.get('stride',1) > 1 else ""
            )
        else:
            st.info("먼저 모델을 슬라이싱하세요.")

    with tab_stl:
        if st.session_state.get("mesh") is not None:
            st.plotly_chart(plot_trimesh(st.session_state.mesh, height=820),
                            use_container_width=True, key="stl_chart", config={'displayModeBar': False})
        else:
    ...
    with tab_gcode:
        if st.session_state.get("gcode_text"):
            ...
            st.info("G-code가 생성되지 않았습니다.")

    if not KEY_OK:
        st.warning("유효한 Access Key가 필요합니다. 만료된 키는 G-code 생성을 허용하지 않습니다.")

# ---- Rapid UI ----
st.sidebar.markdown("---")
if KEY_OK:
    if st.sidebar.button("Generate Rapid(MODX)", use_container_width=True):
    ...
    if st.session_state.show_rapid_panel:
        with st.sidebar.expander("Rapid Settings", expanded=True):
            ...
            rz_preset = st.selectbox("Rz (deg) preset", options=[0.0, 90.0, -90.0],
                                     index=[0.0, 90.0, -90.0].index(float(st.session_state.get("rapid_rz", 0.0))) if float(st.session_state.get("rapid_rz", 0.0)) in [0.0, 90.0, -90.0] else 0)
            st.session_state.rapid_rz = float(rz_preset)

        with st.sidebar.expander("External Axis (A1/A2) 공통", expanded=True):
            st.caption("A1은 raw X, A2는 raw Y 기준으로 작동합니다.\n단축키 A1은 항상 X축, A2는 항상 Y축입니다.\n절대 범위 X (xmin~xmax) 안에서 A1이 선형 보간됩니다.")
            st.session_state.ext_const_speed_mm_s = st.number_input(
                "Axis speed base (mm/s)", min_value=1.0, max_value=2000.0,
                value=float(st.session_state.ext_const_speed_mm_s), step=10.0, format="%.1f"
            )
            st.session_state.ext_const_eps_mm = st.number_input(
                "Boundary snap/hold eps (mm)", min_value=0.0, max_value=50.0,
                value=float(st.session_state.ext_const_eps_mm), step=0.1, format="%.2f"
            )
            st.session_state.ext_const_apply_print_only = st.checkbox(
                "Apply printing-only blocks (E 압출구간만)", value=bool(st.session_state.ext_const_apply_print_only)
            )

            # A2 Step Mode UI (항상 보이도록 이동)
            if "ext_const_a2_use_step" not in st.session_state:
                st.session_state.ext_const_a2_use_step = False
            if "ext_const_a2_step_mm" not in st.session_state:
                st.session_state.ext_const_a2_step_mm = 60.0

            st.session_state.ext_const_a2_use_step = st.checkbox(
                "A2 step mode (uses Y+A4)", value=bool(st.session_state.ext_const_a2_use_step)
            )
            st.session_state.ext_const_a2_step_mm = st.number_input(
                "A2 step length (Y+A4) (mm)", min_value=0.0, max_value=10000.0,
                value=float(st.session_state.ext_const_a2_step_mm), step=1.0, format="%.3f",
                disabled=not bool(st.session_state.ext_const_a2_use_step)
            )

            st.session_state.ext_const_travel_interp = st.checkbox(
                "Interpolate across travel blocks", value=bool(st.session_state.ext_const_travel_interp)
            )

            st.markdown("**(A1) raw X 축 기준 (A1 매핑)**")
            st.session_state.ext_const_enable_a1 = st.checkbox(
                "Enable A1 constant-speed profile", value=bool(st.session_state.ext_const_enable_a1)
            )
            cols = st.columns(2)
            st.session_state.ext_const_xmin = cols[0].number_input("X_min (mm)", value=float(st.session_state.ext_const_xmin), step=50.0, format="%.3f")
            st.session_state.ext_const_xmax = cols[1].number_input("X_max (mm)", value=float(st.session_state.ext_const_xmax), step=50.0, format="%.3f")
            cols2 = st.columns(2)
            st.session_state.ext_const_a1_at_xmin = cols2[0].number_input("A1 at X_min", value=float(st.session_state.ext_const_a1_at_xmin), step=50.0, format="%.3f")
            st.session_state.ext_const_a1_at_xmax = cols2[1].number_input("A1 at X_max", value=float(st.session_state.ext_const_a1_at_xmax), step=50.0, format="%.3f")

            st.markdown("**(A2) raw Y 축 기준 (A2 매핑)**")
            st.session_state.ext_const_enable_a2 = st.checkbox(
                "Enable A2 constant-speed profile", value=bool(st.session_state.ext_const_enable_a2)
            )
            cols3 = st.columns(2)
            st.session_state.ext_const_ymin = cols3[0].number_input("Y_min (mm)", value=float(st.session_state.ext_const_ymin), step=50.0, format="%.3f")
            st.session_state.ext_const_ymax = cols3[1].number_input("Y_max (mm)", value=float(st.session_state.ext_const_ymax), step=50.0, format="%.3f")
            cols4 = st.columns(2)
            st.session_state.ext_const_a2_at_ymin = cols4[0].number_input("A2 at Y_min", value=float(st.session_state.ext_const_a2_at_ymin), step=50.0, format="%.3f")
            st.session_state.ext_const_a2_at_ymax = cols4[1].number_input("A2 at Y_max", value=float(st.session_state.ext_const_a2_at_ymax), step=50.0, format="%.3f")

        with st.sidebar.expander("Mapping Presets (A3/A4)", expanded=False):
            st.caption("Rz가 0, 90, -90일 때만 XYZ, A3, A4 매핑이 동작합니다.")
            ...
            if up_json is not None:
                try:
                    ...
                    if isinstance(loaded, dict) and all(k in loaded for k in ("0","90","-90")):
                        ...
                        st.error("잘못된 프리셋 형태입니다. keys: 0,90,-90 필요")
                except Exception as e:
                    st.error(f"오류: {e}")

            def edit_axis(title_key: str, axis_key: str):
                st.write(f"**Rz {title_key}**, {axis_key} 매핑")
                cols = st.columns(4)
                ...
                if axis_key == "X":
                    cols2 = st.columns(2)
                    if "A4_out" in PAX:
                        a4_0 = cols2[0].number_input("A4_out[0] (X)", value=float(PAX.get("A4_out", [0.0,0.0])[0]), step=50.0, format="%.1f", key=f"{title_key}_X_a4_0")
                        a4_1 = cols2[1].number_input("A4_out[1] (X)", value=float(PAX.get("A4_out", [0.0,0.0])[1]), step=50.0, format="%.1f", key=f"{title_key}_X_a4_1")
                        PAX["A4_out"] = [float(a4_0), float(a4_1)]
                    else:
                        cols2[0].info("A4_out이 X에 없음")
                elif axis_key == "Y":
                    cols2 = st.columns(2)
                    if "A4_out" in PAX:
                        a4_0 = cols2[0].number_input("A4_out[0] (Y)", value=float(PAX.get("A4_out", [0.0,0.0])[0]), step=50.0, format="%.1f", key=f"{title_key}_Y_a4_0")
                        a4_1 = cols2[1].number_input("A4_out[1] (Y)", value=float(PAX.get("A4_out", [0.0,0.0])[1]), step=50.0, format="%.1f", key=f"{title_key}_Y_a4_1")
                        PAX["A4_out"] = [float(a4_0), float(a4_1)]
                    else:
                        cols2[0].info("A4_out이 Y에 없음")
                else: # Z
                    cols2 = st.columns(2)
                    if "A3_out" not in PAX: PAX["A3_out"] = [0.0, 0.0]
                    a3_0 = cols2[0].number_input("A3_out[0]", value=float(PAX.get("A3_out", [0.0,0.0])[0]), step=50.0, format="%.1f", key=f"{title_key}_Z_a3_0")
                    a3_1 = cols2[1].number_input("A3_out[1]", value=float(PAX.get("A3_out", [0.0,0.0])[1]), step=50.0, format="%.1f", key=f"{title_key}_Z_a3_1")
        ...
        if g_txt is not None:
            xyz_count = _extract_xyz_lines_count(g_txt)
        ...
        if g_txt is None:
            st.sidebar.info("먼저 Generate G-Code를 눌러 G-code를 생성하세요.")
        elif over:
            st.sidebar.error("G-code 줄 수가 64,000줄을 초과하여 Rapid 변환이 불가능합니다.")
        elif save_rapid_clicked:
            st.session_state.rapid_text = gcode_to_cone_1500_module(
                g_txt,
            ...
            st.sidebar.success(f"Rapid(.MODX) 변환 성공 (Rz={st.session_state.rapid_rz:.2f})")
