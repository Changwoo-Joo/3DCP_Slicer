...
def _extract_centerline_if_thin(seg3d_closed: np.ndarray, max_thickness_mm: float = 0.1):
    seg3d_closed = np.asarray(seg3d_closed, dtype=float)
    if seg3d_closed.ndim != 2 or seg3d_closed.shape[0] < 4: return seg3d_closed, False
    xy = seg3d_closed[:, :2]
    z_val = float(np.mean(seg3d_closed[:, 2])) if seg3d_closed.shape[1] >= 3 else 0.0
    if np.linalg.norm(xy[0] - xy[-1]) > 1e-9: xy = np.vstack([xy, xy[0]])
    try: poly = Polygon(xy)
    except: return seg3d_closed, False
    if poly.is_empty or not poly.is_valid or poly.area <= 1e-12: return seg3d_closed, False
    try: inward = poly.buffer(-(float(max_thickness_mm) / 2.0), join_style=2)
    except: return seg3d_closed, False
    if not inward.is_empty: return seg3d_closed, False
    coords = np.asarray(poly.exterior.coords, dtype=float)
    if len(coords) < 3: return seg3d_closed, False
    sample = coords
    if len(sample) > 200:
        sample = sample[np.linspace(0, len(sample) - 1, 200).astype(int)]
    diff = sample[:, None, :] - sample[None, :, :]
    d2 = np.sum(diff * diff, axis=2)
    i, j = np.unravel_index(int(np.argmax(d2)), d2.shape)
    idx_start = int(np.argmin(np.linalg.norm(coords - sample[i], axis=1)))
    idx_end = int(np.argmin(np.linalg.norm(coords - sample[j], axis=1)))
    ...
    dedup = [chosen[0]]
    for pt in chosen[1:]:
        if np.linalg.norm(pt - dedup[-1]) > 1e-9: dedup.append(pt)
    chosen = np.asarray(dedup, dtype=float)
    if len(chosen) < 2: return seg3d_closed, False
            ...
            for i_seg, seg3d in enumerate(segments):
                ...
                if thin_wall_as_line:
                    centerline_path, is_thin = _extract_centerline_if_thin(
                        seg3d_no_dup, max_thickness_mm=float(thin_wall_threshold)
                    )

                if is_thin:
                    # --- 얇은 벽 단일선 처리 (옵션 ON일 때만 실행) ---
                    simplified = simplify_segment(centerline_path, float(min_spacing))
                    if len(simplified) >= 2:
                        ref_xy = np.array(ref_pt_layer[:2], dtype=float)
...
with st.sidebar.expander("얇은 벽 단일선 옵션", expanded=False):
    thin_wall_as_line = st.checkbox("얇은 벽 단일 선 경로 처리", value=False, help="체크 시 설정 두께 이하의 얇은 벽을 외곽 루프가 아니라 단일 선 경로로 처리합니다.")
