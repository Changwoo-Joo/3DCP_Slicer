...
def generate_gcode(mesh, z_int=30.0, feed=2000, ref_pt_user=(0.0, 0.0),
                   ...
                   seq_print=False):  # 인자 추가됨
                   
    g = ["; *** Generated by 3DCP Slicer ***", "G21", "G90"]
    if e_on:
        g.append("M83")

    # 옵션에 따라 메쉬 분할 처리 (순차출력이면 분리, 아니면 통째로 리스트에 담음)
    if seq_print:
        sub_meshes = mesh.split(only_watertight=False)
        sub_meshes = sorted(sub_meshes, key=lambda m: m.bounds[0][0])
        g[0] = "; *** Generated by 3DCP Slicer (Sequential Printing) ***"
    else:
        sub_meshes = [mesh]

    safe_z_clearance = float(mesh.bounds[1][2]) + 50.0  # 안전높이 계산
    ...
    for sub_idx, sub_mesh in enumerate(sub_meshes):
        if seq_print:
            g.append(f"\\n; ==========================================")
            g.append(f"; Object {sub_idx + 1} of {len(sub_meshes)}")
            g.append(f"; ==========================================")
        ...
        # 순차 출력일 때만 다음 객체로 넘어가기 전에 노즐을 안전 높이로 들어올림
        if seq_print and sub_idx < len(sub_meshes) - 1:
...
seq_print = st.sidebar.checkbox("순차 출력 (객체별 1개씩 끝까지 인쇄)", value=False) 
