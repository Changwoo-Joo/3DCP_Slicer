...
# =========================
# G-code generator (패치 적용)
# =========================
def generate_gcode(
    mesh, z_int=30.0, feed=2000, ref_pt_user=(0.0, 0.0),
    ...
    prev_start_xy = None      # auto_start 기준점(레이어 첫 시작)
    prev_end_xy = None        # ★ 마지막 출력점(세그먼트 끝점)
    ...
    for z in z_values:
        ...
        for i_seg, seg3d in enumerate(segments):
            ...
            prev_start_xy = simplified[0][:2]   # auto_start용
            prev_end_xy   = simplified[-1][:2]  # ★ 마지막 출력점
